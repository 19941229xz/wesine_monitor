...
public class EventController {
	...
	@ResponseBody
	public Map<String, Object> getEventData(String status, String areaID, String usrName, String usrID, String shopID,
			String roleId, String cPage, String pageSize) {
		...
		case "1":
			...
			conditionMap.put("offset", (cpage - 1) * pagesize);
			conditionMap.put("pageSize", pagesize);
			conditionMap.put("pageTotal", cpage*pagesize);
			// resultMap.put("eventsForThisMonth",
			// eventMapper.selectEventThisMonth3(conditionMap));
			resultMap.put("eventsForThisWeek", eventService.getEvenArrThisWeek3(usrID));//
			// resultMap.put("eventsForThisDay",
			// eventMapper.selectEventThisDay3(conditionMap));
			// status 0 未处理 1暂存 2完成
			
//			conditionMap.put("status", "未处理");
//			resultMap.put("wclTotal", eventMapper.getWclTotal(conditionMap));
//			conditionMap.put("status", "暂存");
//			resultMap.put("zcTotal", eventMapper.getZcTotal(conditionMap));
//			conditionMap.put("status", "完成");
//			resultMap.put("wcTotal", eventMapper.getWcTotal(conditionMap));
...
//			}
		...
		case "2":
			...
			// resultMap.put("eventsForThisWeek",
			// eventService.getEvenArrThisWeek2(areaID));//
			// resultMap.put("eventsForThisDay",
			// eventMapper.selectEventThisDay2(conditionMap));
			// status 0 未处理 1暂存 2完成
			conditionMap.put("status", "未处理");
	...
	@ResponseBody
	public Map<String, Object> saveEditResult(String id, String comment, String productName, String money,
	...
	@ResponseBody
	public Map<String, Object> createActionTime(@RequestParam Map<String, Object> params) {// 包含参数
																							// shop_id
																							// process_person_id
																							// event_id
		...
		if (isNewTAT || Long.parseLong(eventTATList.get(0).get("actionTime") + "") == 0) {// 为被防损员接管
																							// 修改actionTime
	...
	public Map<String, Object> createEndTime(@RequestParam Map<String, Object> params) {
		...
		if ((resultMap.get("status") + "").equals("0")) {// 成功 该防损员处理事件次数加1
			...
			if (fsyList.size() == 0) {// 不存在 该用户的fsy信息 需要重新生成
				...
				if ((num2 + "").equals("999999999999999999")) {
					num2 = 0;
				}
	...
	@RequestMapping(value="/getEventTotalNumByStatus")
	@ResponseBody
	public Map<String,Object> getEventTotalNumByStatus(@RequestParam Map<String,Object> params){
		resultMap=new HashMap<String,Object>();
		//conditionMap=new HashMap<String,Object>();
		String cstatus="";
		...
		int num=eventMapper.getEventTotalNumByStatus(params);
